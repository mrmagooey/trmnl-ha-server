...
- `gap_style` (history_graph only, optional): how to draw an outage — a stretch where Home Assistant reported the entity as `unavailable` or `unknown` between two real readings. The graph never interpolates straight across one, since that would show a trend nobody measured. Default `hold`.
  ...
  | Value | Drawn as |
  |-------|----------|
  | `hold` | A dashed line holding the last known value flat across the outage. The jump to the recovered value is left undrawn, so the line reads as two pieces. |
  | `break` | Nothing at all. The line stops when the entity goes away and restarts when it returns. |
  | `step` | As `hold`, plus a dashed vertical riser joining the held value to the recovered one, so the series stays connected as a step. |

  This setting covers outages *between* readings. An entity that stops reporting and has not come back still has its last value held forward as a dotted tail to the right edge under every style.

  ```yaml
  - entity_name: "sensor.outdoor_temperature"
    friendly_name: "Outdoor"
    type: "history_graph"
    gap_style: "step"
  ```
